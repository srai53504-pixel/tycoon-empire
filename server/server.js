...
// PostgreSQL is used as the shared global ranking store. The local JSON file



// remains the gameplay compatibility store, while this table is authoritative



// for cross-device ranking.
...
async function initGlobalRankingDatabase() {
  ...
  if (!pgPool) {
    ...
    console.warn("DATABASE_URL is not set; global ranking will use local fallback only.");
  ...
  await pgPool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_global_companies_net_worth



    ON global_companies(net_worth DESC, xp DESC, company_id ASC)
  ...
  console.log("PostgreSQL global ranking database ready.");
...
async function syncGlobalCompany(company) {



  if (!pgPool || !company) return;
  ...
  await pgPool.query(`
    ...
    INSERT INTO global_companies
      ...
      (company_id, company_name, owner_id, country, money, net_worth, level, xp, updated_at)
  ...
  `, [
    ...
    String(company.company_name ?? company.name ?? `Company ${companyId}`),



    company.owner_id != null ? String(company.owner_id) : company.ownerId != null ? String(company.ownerId) : null,



    String(company.country ?? "India"),



    Number(company.money ?? 0),



    Number(company.net_worth ?? company.netWorth ?? 0),



    Number(company.level ?? 1),
...
async function syncAllGlobalCompanies() {
  ...
  for (const company of Object.values(db.legacy?.companies || {})) {
    ...
    try {
      ...
      await syncGlobalCompany(company);
      ...
      console.error("Global ranking sync failed:", e.message);
  ...
  for (const company of Object.values(db.companies || {})) {
    ...
    try {
      ...
      await syncGlobalCompany(normalized);
      ...
      console.error("Modern company ranking sync failed:", e.message);
...
async function getGlobalRanking(companyId = null) {
  ...
  const result = await pgPool.query(`
    ...
    WITH ranked AS (
      ...
      SELECT
        ...
        RANK() OVER (ORDER BY net_worth DESC, xp DESC, company_id ASC) AS world_rank,
    ...
    ${companyId == null ? "" : "WHERE company_id=$1"}
  ...
  return result.rows.map(row => ({
    ...
    ownerName: db.users?.[row.owner_id]?.username || "Unknown",
...
app.get("/api/rankings", auth, async (req, res) => {
  ...
  try {
    ...
    await syncAllGlobalCompanies();



    const rankings = await getGlobalRanking();
    ...
    if (rankings) {
      ...
      return res.json({
        ...
        total_companies: rankings.length
    ...
    const fallback = Object.values(db.companies)
    ...
    return res.json({
      ...
      rankings: fallback.map((r, i) => ({ ...r, rank: i + 1 })),



      total_companies: fallback.length
    ...
    return res.status(500).json({ error: "Unable to load global rankings" });
...
app.all(['/RestSimulator','/RestSimulator/*'], async (req,res)=>{
  ...
  try {
    ...
    switch(op){
      ...
      case 'getUserData': { const c=ensureLegacyCompany(qp(req,'company_id','0')); await syncGlobalCompany(c); const ranked=await getGlobalRanking(c.company_id); const r=ranked?.[0]; ok(res,{...legacyView(c),compnay_id:Number(c.company_id),ranking:r?.ranking||0,world_rank:r?.worldRank||0,total_companies:r?.totalCompanies||0}); break; }



      case 'updateUserData': { const c=ensureLegacyCompany(qp(req,'company_id','0')); updateFromParams(c,req); saveDB(); await syncGlobalCompany(c); const ranked=await getGlobalRanking(c.company_id); const r=ranked?.[0]; ok(res,{...legacyView(c),compnay_id:Number(c.company_id),ranking:r?.ranking||0,world_rank:r?.worldRank||0,total_companies:r?.totalCompanies||0}); break; }
      ...
      case 'getWorldRankings': case 'getRankings': case 'getGlobalRankings': {
        ...
        ok(res,{rankings:rankings||[],total_companies:rankings?.length||0});
...
(async () => {
  ...
  try {
    ...
    await initGlobalRankingDatabase();
